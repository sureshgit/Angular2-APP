...
(function (window, $) {
    ...
    angular.module('ngGrid.services').factory('$domUtilityService', ['$utilityService', function ($utils) {
        ...
        domUtilityService.UpdateGridLayout = function ($scope, grid) {
            ...
            grid.elementDims.rootMaxH = grid.$root.parent().height() - 8;// grid.config.multiSelect ? grid.$root.height() : grid.$root.parent().height() - 8;
    ...
    var ngFooter = function ($scope, grid) {
        ...
        //    if ($scope.maxRows())
        //        return new Array(1);
        //    return new Array(Math.ceil($scope.maxRows() / $scope.pagingOptions.pageSize));
        //};

        $scope.pageForward = function () {
            var page = $scope.pagingOptions.currentPage;
            if ($scope.totalServerItems > 0) {
                $scope.pageChangeEvent(Math.min(page + 1, $scope.maxPages()));
                //$scope.pagingOptions.currentPage = Math.min(page + 1, $scope.maxPages());
            } else {
                $scope.pageChangeEvent($scope.pagingOptions.currentPage++);
        ...
        $scope.pageBackward = function () {
            var page = $scope.pagingOptions.currentPage;
            $scope.pageChangeEvent(Math.max(page - 1, 1));
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
        ...
        self.sortActual = function () {
            if (!self.config.useExternalSorting) {
                ...
                tempData = tempData.sortByKey(self.config.sortInfo.fields[0], self.config.sortInfo.directions[0]);
        ...
        $scope.totalFilteredItemsLength = function () {
            if (!$scope.pagesNumberArray || $scope.pagesNumberArray.length <= 0)
                $scope.pagesNumberArray = $scope.buildPaging();//new Array(Math.ceil(($scope.totalServerItems ? $scope.totalServerItems : self.filteredRows.length) / $scope.pagingOptions.pageSize));
            return self.filteredRows.length;
        };

        $scope.buildPaging = function (curPage) {
            var curPageNumber = curPage ? curPage : $scope.pagingOptions.currentPage;
            var result = [];
            var totalPageCount = Math.ceil(($scope.totalServerItems ? $scope.totalServerItems : self.filteredRows.length) / $scope.pagingOptions.pageSize);
            if (totalPageCount > 7) {
                $scope.addRange(1, 2, result);
                if (curPageNumber > 4) {
                    $scope.addDots(result);
                }
                else {
                    $scope.addRange(3, 5, result);
                }

                if (curPageNumber > 4 && curPageNumber < totalPageCount - 3)
                    $scope.addRange(curPageNumber - 1, curPageNumber + 1, result);

                if (curPageNumber < totalPageCount - 3)
                    $scope.addDots(result);
                else {
                    $scope.addRange(totalPageCount - 4, totalPageCount - 2, result);
                }
                $scope.addRange(totalPageCount - 1, totalPageCount, result);
                return result;
            }
        ...
        $scope.addRange = function (start, finish, list) {
            var i = 0;
        ...
        $scope.pageChangeEvent = function (value) {
            if (!value)
                return;
            //if ($scope.pagesNumberArray && $scope.pagesNumberArray.length > 7)
            $scope.pagesNumberArray = $scope.buildPaging(value);
            $scope.pagingOptions.currentPage = value;
            $scope.pagingOptions.gridId = self.gridId;
        }
    ...
    var ngRow = function (entity, config, selectionProvider, rowIndex, $utils) {
        ...
        this.dblClickFn = config.dblClickFnCallback;
    ...
    ngRow.prototype.dblClick = function (event) {
        if (this.dblClickFn)
            this.dblClickFn(this);
    };

    ngRow.prototype.toggleSelected = function (event, row) {

        try {
            if (dbClickForTouchDevice(row, event) == true) {
                return;
            }
        }
        catch (error)
    ...
    var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
        ...
        self.rowConfig = {
            ...
            dblClickFnCallback: grid.config.dblClickFn,
            ...
            compile: function () {
                return {
                    pre: function ($scope, iElement) {
                        ...
                        var fieldExpression = "";
                        if ($scope.col.field)
                            fieldExpression = 'row.entity.' + $scope.col.field;

                        var cellTemplate = $scope.col.cellTemplate.replace(COL_FIELD, fieldExpression);

                        if ($scope.col.enableCellEdit) {
                            html = $scope.col.cellEditTemplate;
                            html = html.replace(DISPLAY_CELL_TEMPLATE, cellTemplate);
                            html = html.replace(EDITABLE_CELL_TEMPLATE, $scope.col.editableCellTemplate.replace(COL_FIELD, fieldExpression));
                        } else {
                            html = cellTemplate;
                        }
                        ...
                        return grid.init().then(function () {
                            ...
                            if (typeof options.totalServerItems === "string") {
                                $scope.$parent.$watch(options.totalServerItems, function (newTotal, oldTotal) {
                                    ...
                                    if (!angular.isDefined(newTotal)) {
                                        ...
                                        $scope.pagesNumberArray = $scope.buildPaging();
                                    }
                                });
                            }
                            ...
                            if (typeof options.data === "string") {
                                var dataWatcher = function (a) {
                                    ...
                                    grid.configureColumnWidths();
                                    grid.refreshDomSizes();
                                    //When i click on live method statement accordian panel.A call goes to get the data once data come and stored in array . sort watch is written whcih monitor array changes for every field or column and for each record which comes from server so it goes to infinite watch or $apply that is why page keep on loading.
                                    //where as when we click on  any field  to sort it sort the specific column data not all
    ...
    angular.module("ngGrid").run(["$templateCache", function ($templateCache) {
        ...
        $templateCache.put("aggregateTemplate.html",
          ...
          "    <span class=\"ngAggregateText\">{{row.label CUSTOM_FILTERS}} ({{row.totalChildren()}})</span>" +
        ...
        $templateCache.put("footerTemplate.html",
          ...
          "            </select>" +
          "        </div>" +
          "        <ul ng-show=\"pagesNumberArray && pagesNumberArray.length>1 && totalServerItems>0\" style=\"float:left; margin-right: 10px; line-height:25px;\" class=\"ngPagerControl pagination\" style=\"float: left; min-width: 135px;\">" +
          "            <li><a href=\"javascript:void(0)\" ng-click=\"pageBackward()\" ng-disabled=\"cantPageBackward()\" title=\"{{i18n.ngPagerPrevTitle}}\">&#171;</a></li>" +
          "            <li ng-class=\"{active : pagingOptions.currentPage === item.value }\"  ng-repeat=\"(i,item) in pagesNumberArray track by $index\" style=\"cursor:pointer\">" +
          "            <a ng-click=\"pageChangeEvent(item.value)\">{{item.text}}" +
          "            </a></li>" +
          "            <li><a href=\"javascript:void(0)\" class=\"\" ng-click=\"pageForward()\" ng-disabled=\"cantPageForward()\" title=\"{{i18n.ngPagerNextTitle}}\">&#187;</a></li>" +
          "        </ul>" +
          "    </div>" +
          "</div>"
        );
        ...
        $templateCache.put("gridTemplate.html",
          ...
          "        <div ng-style=\"rowStyle(row)\" ng-repeat=\"row in renderedRows\" ng-click=\"row.toggleSelected($event, row);\" ng- ng-dblclick=\"row.dblClick($event);\" ng-class=\"row.alternatingRowClass()\" ng-row></div>" +
          "    </div>" +
          "</div>" +
