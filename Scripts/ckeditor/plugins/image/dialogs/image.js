...
(function () {
    var imageDialog = function (editor, dialogType) {
        ...
        var images = [];
        var globalImages = [];
        var citationId = "89504e36-557b-4691-8f1b-7e86f9cf95ea";
        var cid = getParameterByName("cid");
        if (cid)
            getLocationImages(cid).then(function (data) {
                images = data;
            });
        getLocationImages(citationId).then(function (data) {
            globalImages = data;
        });;

        function getLocationImages(cid) {
            return $.ajax({
                url: ("/api/document?filterDocumentExtension=.jpg,.png&" + (cid ? ("cid=" + cid) : "")),
                dataType: 'json',
                data: ""
            });
        }
        ...
        function buildImagesSelect(dialog) {
            var mainDiv = $("<div class='imageSelector' style='top:" + dialog[0].style.top + "; left:" + dialog[0].style.left + "; width:" + dialog.width() + "px; height:" + dialog.height() + "px; '></div>");

            var buttonDiv = $("<div class='buttonGroup'></div>");
            var button = $("<a href='javascript:void(0)' class='cke_dialog_ui_button' ><span  class='cke_dialog_ui_button'>Select</span></a>");
            var closeButton = $("<a href='javascript:void(0)' class='cke_dialog_ui_button' ><span  class='cke_dialog_ui_button'>Close</span></a>");
            closeButton.click(function () {
                $(".imageSelector").remove();
            });

            var containerDiv = $("<div class='imageContainer'></div>");
            if (images && images.length > 0)
                containerDiv.append(buildImagesContainer(images, "Company images", cid));
            containerDiv.append(buildImagesContainer(globalImages, "Citation images", citationId));

            mainDiv.append(containerDiv);

            button.click(function () {
                var selectedUrl = $(".imageSelector .selected img").attr("src");
                $(".linkUrl input").val(selectedUrl);
                $(".imageSelector").remove();
            });
        ...
        function buildImagesContainer(items, headerText, companyId) {
            var mainDiv = $("<div class='companyBlock'></div>");
            var headerDiv = $("<div class='containerHeader'></div>");
            headerDiv.text(headerText);
            var ul = $("<ul></ul>");

            var urlSplitted = window.location.href.split("/");
            var imgHost = urlSplitted[0] + "//" + urlSplitted[2];

            for (var i = 0; i < items.length; i++) {
                var li = $("<li></li>");
                var img = $("<img ></img>");
                img.attr('src', (imgHost + '/document.atld?id=' + items[i].Id + (companyId ? "&cid=" + companyId : "")));
                img.attr('imageId', items[i].Id);
                li.click(function () {
                    $(".imageSelector li").removeClass("selected");
                                    ...
                                    children: [
                                        ...
                                        },
                                        {
                                            ...
                                            onClick: function (sender) {
                                                var elm = $("#" + sender.sender.domId);
                                                var dialog = elm.parents(".cke_dialog");
                                                var htmlImages = buildImagesSelect(dialog);
                                                elm.parents(".cke_dialog_contents").append(htmlImages);
                                            }
                                        }
                                    ]
                                }
